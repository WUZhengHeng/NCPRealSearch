...
$.ajax({
	url: 'https://jk.avuejs.com/news?name=山东省',
	dataType: 'json',
	success: (data) => {
		let str = '';
		data.forEach(ele => {
...
$.ajax({
	...
	success: (res) => {
		...
		let number = list.findIndex(ele => ele.name === '山东');
		let province= list[number];
		...
		if (province.name === '山东') {
			$('#count11').html(province.total.confirm);
			$('#count21').html(province.total.suspect);
			$('#count31').html(province.total.heal);
			$('#count41').html(province.total.dead);
		}
		$('#list').html(str)
	}
})

...
$(document).ready(function() {
	...
	function addFunction() {
		var $pop_qs = $('.pop_qs');
		var $qs_mask = $('.qs_mask');
		var $pop_technical = $('.pop_technical');
...
function showPlaceCardNews(area) {
	$.ajax({
		url: 'https://api.dreamreader.qq.com/news/v1/province/news/list?province_code=' + area || 'sd',
		type: 'get',
		scriptCharset: 'UTF-8',
		success: function(res) {
			if (res.data.items) {
